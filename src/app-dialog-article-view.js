...
const component = {
    ...
    template: `
        <v-dialog v-bind="state" fullscreen data-lenis-prevent :class="display.xs.value ? 'ma-4' : 'ma-16'" @update:modelValue="(v) => methods.update(v)">
            <v-card :loading="state.loading" :disabled="state.loading">
                <v-toolbar class="position-fixed" color="transparent">
                    <v-toolbar-title></v-toolbar-title>
                    <v-toolbar-items>
                        <v-btn icon="mdi-close" href="#works" @click="methods.close()" />
                    </v-toolbar-items>
                </v-toolbar>
                <v-fade-transition mode="out-in">
                    <v-card-text v-if="!state.loading" v-html="state.content" />
                    <!-- <v-skeleton-loader v-else type="image, article, text"></v-skeleton-loader> -->
                </v-fade-transition>
            </v-card>
        </v-dialog>
    `

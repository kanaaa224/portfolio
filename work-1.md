...
ハードウェア選定からOSのインストール、ネットワーク構成、セキュリティ対策、サービスの公開までを一貫して自分で行なっています。

サーバーでは、自作したWebアプリやWebAPIが動作しており、デプロイ時は厳格に検証したり、  
セキュリティにも万全を期しているので、今まで攻撃によって問題が起きた事はありません。
...
・ソフトウェア/ミドルウェア  
Windows 11 Server | IIS | Docker Compose | GitHub Actions | Node.js / FastAPI / PHP / Vue.js
...
2022/08:  
ゲームサーバーの利用人数が最大30人になることがあり、頻繁にゲームサーバーが落ちてしまい、  
その都度起動し直す必要が出てきたため、管理者不在でも遠隔で起動したり設定できるWebアプリ
「[サーバーステータス](https://ponzu.server-on.net/status/?from=portfolio)」を開発し、デプロイしました
...
2025/12:  
インストールされていたOSがサポート切れとなっていたため、OSのクリーンインストール  
それに伴い、Webの実行環境をApacheからIISへ移行
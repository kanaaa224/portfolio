...
2022/08:  
フロントエンド部分が完成  
ゲームサーバーの起動・停止操作機能が完成したバージョン v1 をデプロイ

2022/10:  
ゲームサーバーの設定編集機能を追加

2023/09:  
自作 WebAPI が完成し、バックエンド処理を移行（アプリとバックエンドの分離、依存を解消）  
APIで障害が起きても、アプリの実行は可能

2024/08:  
Vue.jsを採用し、動作のパフォーマンス向上（DevToolsなどで計測しながら最適化）

### 想定されるユースケース

・管理者が不在  
→ パスワードを知っている人が起動などの操作を実行可能

・友人から「マイクラ鯖立ててー」と頼まれた  
→ その友人はアプリから起動ボタンを押すだけ

・サーバーの動作が重い  
→ ステータスを確認し、場合によって再起動を実施

・設定を一時的に変えたい  
→ 設定ダイアログから設定ファイルを編集・保存